...
SOURCE = ROOT / "assets/source/mikel.png"
ASSETS = ROOT / "assets"
...
ROWS = [
    ("Subject", "Mikel"),
    ("Role", "Software Developer AI/ML & Data Science"),
    ("Origin", "Barakaldo Spain"),
    ("Status", "Building + Learning + Shipping"),
    ("ToolChain", "VSCode · Cursor · Git"),
    ("Core.Lang", "Python · Java · SQL"),
    ("Core.AI", "RAG · LLMs · XGBoost"),
    ("Core.Backend", "FastAPI · Spring"),
    ("Core.Data", "Postgres · MongoDB · FAISS"),
    ("Core.Infra", "Docker · Linux"),
    ("Grid.GitHub", "mikelrh-dev"),
    ("Grid.LinkedIn", "/in/mikel-romero-homobono-760878337"),
    ("Grid.Mail", "—"),
]
...
def make_logos() -> dict[str, Image.Image]:
    ...
    # Data-inspired four-point star with a small connected node network.
    data = Image.new("RGBA", (size, size), (0, 0, 0, 0))
...
def portrait_points(theme: str, rng: np.random.Generator) -> np.ndarray:
    ...
    # Proportional head + shoulders crop: aspect 372:422, centred horizontally,
    # covering full height so any portrait fills the VISUAL.MAP frame.
    w, h = source.size
    bw = min(w, int(h * 372 / 422))
    x0 = (w - bw) // 2
    crop = source.crop((x0, 0, x0 + bw, h)).resize((300, 340), Image.Resampling.LANCZOS)
...
def render_svg(
    ...
    data = transport(code, logo_points["data"][:n])
    ...
    frames = [source, source, rust, rust, code, code, data, data, source]
    ...
    parts: list[str] = [
        ...
        "<title id=\"title\">Mikel's live system profile</title>",
        '<desc id="desc">Animated terminal profile with a dithered portrait and '
        "code, Python, and data silhouettes.</desc>",
        "<defs>",
    ...
    parts.extend(
        [
            ...
            f'stroke="{t["chrome"]}"/>',
             ...
             'font-weight="700">@mikelrh-dev</text>',
        ]
    )
